...
def _ml_refine_scores(
    ...
    """Lightweight ML: nearest-centroid prototype scoring from seeded branches."""
    ...
    # Blend ML similarity with handcrafted score.
...
def _assign_branches(
    ...
    # Optionally seed missing selected classes only when there is strong evidence.
...
def _smooth_branch_labels(
    ...
    """Prefer local consistency among sibling branches unless confidence is strong."""
    if not branch_class:
        return branch_class
...
def _apply_rules(rows: list[dict[str, Any]], opts: RuleBatchOptions) -> tuple[list[int], list[float], int, int]:
    ...
    # Ensure root is soma when soma rule is enabled.
    if opts.soma:
        for i, row in enumerate(rows):
    ...
    if enabled_neurites:
        ...
        # Initial branch-level assignment.
        for bid, nodes in branch_nodes.items():
        ...
        # Keep soma as soma if requested.
        if opts.soma:
            for i, row in enumerate(rows):
                if int(row["parent"]) == -1:
                    types[i] = 1
